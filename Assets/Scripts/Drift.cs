...
public class Drift : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        else
        {
            ...
            if (transform.position.x > topLeftBoundary.x && xPos.x < 0)
            {
                transform.position = transform.position + xPos;

            }
            else if (transform.position.x < topLeftBoundary.x && xPos.x > 0)
            {
                transform.position = transform.position + xPos;

            }

            if(transform.position.y < topLeftBoundary.y && yPos.y < 0)
            {
                transform.position = transform.position + xPos;
            }
            else if(transform.position.y < bottomRightBoundary.y && yPos.y > 0)
            {
                transform.position = transform.position + xPos;
